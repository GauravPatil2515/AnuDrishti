#!/usr/bin/env python3
"""
BBBP Model Training Script
==========================
Trains Attention-GINet on Blood-Brain Barrier Penetration dataset.

Dataset: BBBP (2,051 molecules)
Task: Binary classification (p_np: 1 = permeable, 0 = non-permeable)
"""
...
class AttentionGINet(nn.Module):
    """Attention-augmented GIN for BBBP"""
...
def train_bbbp():
    ...
    # Load data
    data_dir = project_dir / "MODELS" / "bbbp_model_full_package" / "data" / "bbbp"
    train_data, valid_data, test_data = load_bbbp_data(data_dir)
    ...
    print(f"   Class balance: {pos_count}/{neg_count} (pos/neg)")
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        val_roc, val_acc = evaluate(model, valid_loader, device)
        ...
        if val_roc > best_val_roc:
            ...
            # Save best model
            torch.save(model.state_dict(), output_dir / "bbbp_gin_model.pth")
            print(f"Epoch {epoch:3d} | Loss: {train_loss:.4f} | Val ROC-AUC: {val_roc:.4f} ⭐ NEW BEST")
        else:
            patience_counter += 1
            if epoch % 10 == 0:
    ...
    # Load best model
    model.load_state_dict(torch.load(output_dir / "bbbp_gin_model.pth"))
    
    test_roc, test_acc = evaluate(model, test_loader, device)
    
    print(f"\n✅ Best Validation ROC-AUC: {best_val_roc:.4f} (Epoch {best_epoch})")
    print(f"✅ Test ROC-AUC: {test_roc:.4f}")
    print(f"✅ Test Accuracy: {test_acc:.4f}")
    print(f"\n💾 Model saved to: {output_dir / 'bbbp_gin_model.pth'}")
