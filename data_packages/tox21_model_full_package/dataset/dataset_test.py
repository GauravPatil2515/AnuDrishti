        ...
        for i, row in enumerate(csv_reader):
            if i != 0:
                ...
                if isinstance(target, list):
                    ...
                    # Legacy single target support (optional, but good for backward compat if needed)
                    label = row[target]
                    if label == '':
                        # Skip or handle missing? Original skipped.
                        continue 
                    if task == 'classification':
                        labels.append(int(label))
                    elif task == 'regression':
                        labels.append(float(label))
                    labels.append(label) # This seems wrong in original logic if it was appending twice? 
                                         # Wait, original was:
                ...
                if mol != None:
                    smiles_data.append(smiles)
                    # If we processed list targets above, we already appended to labels.
                    # But we need to sync with valid mols.
                    # Let's refactor slightly to be safer.
                    
    # Re-reading to match original logic structure but with multi-target support
    smiles_data, labels = [], []
    with open(data_path) as csv_file:
        csv_reader = csv.DictReader(csv_file, delimiter=',')
        for i, row in enumerate(csv_reader):
                ...
                if mol is not None:
                    if isinstance(target, list):
                        ...
                        for t in target:
                            val = row[t]
                            if val == '':
                                # Use -1 as placeholder for missing labels (distinct from 0 and 1)
                                label_list.append(-1.0)
                            else:
                                if task == 'classification':
                                    label_list.append(float(val))
                                elif task == 'regression':
                                    label_list.append(float(val))
