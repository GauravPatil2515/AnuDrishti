...
\newcommand{\Fconstrained}{0.941}   % mean faithfulness F, constrained, claim-bearing (n=20)
...
% Unconstrained baseline PENDING: the constrained run exhausted the Groq free-tier
% daily token cap, so the baseline could not run. Re-run on a fresh quota/key.
\newcommand{\Funconstrained}{\TODO{unconstrained F pending}}
\newcommand{\Guncon}{\TODO{?}}
\newcommand{\Cuncon}{\TODO{?}}
...
We evaluated faithfulness with a live LLaMA-3.3-70B model over \Ntoxic{} toxic-predicted Tox21 test molecules (max-endpoint probability $\geq 0.5$), the setting in which a toxicity explanation is meaningful. Of these, \NwithClaims{} yield at least one groundable structural claim; Table~\ref{tab:faith_compare} reports the constrained DeNovo pipeline on this claim-bearing subset. The unconstrained-LLM baseline, scored under the identical metric, is pending: the constrained run exhausted the Groq free-tier daily token budget before the baseline could complete, so those cells are placeholders to be filled on re-run.

\begin{table}[h]
\centering
\caption{Faithfulness of the constrained DeNovo pipeline on Tox21 (\NwithClaims{} claim-bearing molecules of \Ntoxic{} toxic-predicted; live LLaMA-3.3-70B). The unconstrained-LLM baseline column is pending a re-run on a fresh token budget.}
...
Over \Ntoxic{} toxic-predicted molecules, \NwithClaims{} yield at least one groundable structural claim; on these, the constrained mechanism attains a mean faithfulness of \Fconstrained{} (grounding \Gconstr{}, causal consistency \Cconstr{}), while \RejRate{} of generation attempts are rejected as unfaithful. The high grounding score confirms that accepted explanations cite substructures the GNN actually attended to, and the causal component confirms those substructures measurably drive the prediction. We emphasize that a non-trivial rejection rate is a \emph{feature}: each rejected explanation is one that would otherwise have presented a hallucinated rationale to the user as trustworthy. \TODO{Add the unconstrained-LLM baseline comparison once re-run on a fresh token budget (the constrained run exhausted the Groq free-tier daily cap).}
...
We have demonstrated that faithful explainability requires verifying the causal link between LLM-generated text and a model's actual reasoning---not merely generating fluent rationales. Evaluated with a live LLaMA-3.3-70B model over \Ntoxic{} toxic-predicted Tox21 molecules, the proposed counterfactual validation framework attains a mean faithfulness of \Fconstrained{} on the explanations it accepts while rejecting \RejRate{} of generation attempts as unfaithful---all without degrading predictive performance.
