...
\begin{table}[h]
\centering
\caption{Ablation Study on Tox21 (validation ROC-AUC). $^{\dagger}$Mean pooling yields no per-atom attention weights; faithfulness is undefined (N/A) for this configuration.}
\label{tab:ablation}
\begin{tabular}{lcc}
\toprule
...
-- Attention Pooling (Mean)$^{\dagger}$ & 0.815 & N/A \\
-- Transfer Learning & 0.798 & \TODO{re-run} \\
-- Causal Validation (grounding only) & 0.837 & \TODO{re-run} \\
\bottomrule
\end{tabular}
\end{table}
