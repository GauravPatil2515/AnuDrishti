...
\begin{table}[h]
\centering
\caption{Faithfulness Comparison on Tox21 (\NwithClaims{} claim-bearing molecules out of 200 test samples; bootstrap $95\%$ CIs: DeNovo $[0.815, 0.910]$, Unconstrained $[0.005, 0.040]$; per-cell CIs omitted for space).}
...
We have demonstrated that faithful explainability requires verifying the causal link between LLM-generated text and a model's actual reasoning---not merely generating fluent rationales. The proposed counterfactual validation framework raises mean faithfulness from \Funconstrained{} (unconstrained LLM) to \Fconstrained{} over the full test population (bootstrap $95\%$ CI $[0.815, 0.910]$), rejecting \RejRate{} of explanations as unfaithful---a statistically significant improvement achieved without degrading predictive performance.
